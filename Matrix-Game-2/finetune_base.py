"""
Full base model finetuning script with DDP support for multi-GPU training.

This script finetunes all model weights (not LoRA) on new domains.
WARNING: This requires significantly more VRAM and may overwrite the base model's capabilities.

To run on multiple GPUs:
    torchrun --nproc_per_node=NUM_GPUS finetune_base.py

Example with 2 GPUs:
    torchrun --nproc_per_node=2 finetune_base.py
...
python finetune_base.py --latent_frames 3 --overfit --overfit_blocks 7

# Train on 23 consecutive blocks (= 69 latent frames worth of data)
python finetune_base.py --latent_frames 3 --overfit --overfit_blocks 23
"""
...
def parse_args():
    ...
    parser.add_argument("--lr", type=float, default=5e-6,
                        help="Learning rate (5e-6 recommended for style finetuning)")
    ...
    parser.add_argument("--output_dir", type=str, default=None,
                        help="Output directory (default: outputs/finetune_base_<timestamp>)")
...
def get_sequence_config(latent_frames, gpu="rtx6000", gradient_checkpointing=False):
    """
    Config for style finetuning (5 blocks + head = ~270M trainable params).
    ...
    if gradient_checkpointing:
        ...
        # Style mode trains ~270M params (17% of model) - between LoRA and full finetuning
        configs = {
            "a100": {  # 40GB
                3:  {"batch_size": 2, "grad_accum": 6},   # ~28GB, video=9 frames
                6:  {"batch_size": 1, "grad_accum": 12},  # ~35GB, video=21 frames
            },
            "rtx6000": {  # 96GB
                3:  {"batch_size": 4, "grad_accum": 3},   # ~35GB, video=9 frames
                6:  {"batch_size": 2, "grad_accum": 6},   # ~50GB, video=21 frames
                9:  {"batch_size": 2, "grad_accum": 6},   # ~65GB, video=33 frames
                12: {"batch_size": 1, "grad_accum": 12},  # ~75GB, video=45 frames
                15: {"batch_size": 1, "grad_accum": 12},  # ~85GB, video=57 frames
            },
        }
    else:
        # Without gradient checkpointing (faster but more memory)
        # Style mode trains ~270M params (17% of model)
        configs = {
            "a100": {  # 40GB
...
def train_step(model, vae, batch, scheduler, accumulation_steps, device ):
    ...
    # Clip gradients on all trainable parameters
...
def main():
  ...
  output_dir = args.output_dir if args.output_dir else f"outputs/finetune_base_{timestamp}"
  ...
  # ==========================================================================
  # SELECTIVE FINETUNING - Choose which layers to train
  # ==========================================================================
  ...
  # Parse train_targets
  train_targets = args.train_targets.lower()

  if train_targets == "all":
    # Train everything
    for param in model.parameters():
      param.requires_grad = True
    pprint("Training ALL parameters")

  elif train_targets == "later_blocks":
    # Train later blocks + head (head is NOT in frozen list, so it trains)
    # Similar to finetune_ddp.py approach
    frozen_modules = [
      ...
      'action_model',         # Action conditioning
    ]
    # Add first N blocks to frozen list (head is NOT frozen)
    for i in range(args.freeze_first_n_blocks):
      frozen_modules.append(f'blocks.{i}.')

    for name, param in model.named_parameters():
      if not any(fm in name for fm in frozen_modules):
        param.requires_grad = True
    pprint(f"Training later blocks + head (freezing first {args.freeze_first_n_blocks} blocks + embeddings + action_model)")

  elif train_targets == "style":
    # Style mode: trains head + last 5 blocks (25-29) for fixing visual drift
    # This targets the output layers where visual style is encoded
    frozen_modules = [
      'patch_embedding',      # Spatial feature extraction - keep frozen
      'img_emb',              # CLIP visual context - keep frozen
      'time_embedding',       # Diffusion timestep encoding - keep frozen
      'time_projection',      # Time modulation - keep frozen
      'action_model',         # Action conditioning - keep frozen
    ]
    # Freeze first 25 blocks (0-24), train last 5 (25-29) + head
    for i in range(25):
      frozen_modules.append(f'blocks.{i}.')
    ...
    for name, param in model.named_parameters():
      if not any(fm in name for fm in frozen_modules):
        param.requires_grad = True
    pprint("Training STYLE layers (head + blocks 25-29) for visual drift fix")

  else:
    # Parse comma-separated targets: 'self_attn', 'cross_attn', 'ffn'
    target_list = [t.strip() for t in train_targets.split(",")]
    train_self_attn = "self_attn" in target_list
    train_cross_attn = "cross_attn" in target_list
    train_ffn = "ffn" in target_list

    pprint(f"Training targets: self_attn={train_self_attn}, cross_attn={train_cross_attn}, ffn={train_ffn}")

    for name, param in model.named_parameters():
      should_train = False

      # Check if this parameter is in a target layer
      if train_self_attn and 'self_attn' in name:
        should_train = True
      if train_cross_attn and 'cross_attn' in name:
        should_train = True
      if train_ffn and '.ffn.' in name:
        should_train = True
  ...
  pprint(f"Total parameters: {total_params:,}")
  pprint(f"Trainable parameters: {trainable_count:,} ({100.0 * trainable_count / total_params:.2f}%)")
  ...
  hyperparams = {
    ...
    "freeze_first_n_blocks": args.freeze_first_n_blocks if args.train_targets == "later_blocks" else 0,
  ...
  for epoch in range(args.num_epochs):
    ...
    if is_main:
        ...
        if (epoch + 1) % args.checkpoint_frequency == 0:
            checkpoint_path = f"{output_dir}/model_epoch{epoch}.safetensors"
            # Get the unwrapped model (not DDP wrapper)
            raw_model = model.module if hasattr(model, 'module') else model
